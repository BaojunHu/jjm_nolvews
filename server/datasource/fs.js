...
export class FsDataSource {
    ...
    async getVolumes(story) {
        const volsDir = path.join(this.storyDir(story), 'volumes');
        if (!await fs.pathExists(volsDir))
            return [];
    ...
    async getChapters(story, vol) {
        ...
        for (const v of targetVols) {
            ...
            if (!await fs.pathExists(contentDir))
                continue;
            const files = await fs.readdir(contentDir);
            const chapterNums = [...new Set(files.map(f => f.match(/^chapter-(\d+)/)?.[1]).filter(Boolean))];
    ...
    async collectFromAllVolumes(story, filename, fallback, vol) {
        if (vol) {
            const data = await this.readTrackingJson(story, vol, filename, fallback);
            return [{ vol, data }];
        }
        const volumes = await this.getVolumes(story);
        const results = [];
        for (const v of volumes) {
            const data = await this.readTrackingJson(story, v.number, filename, fallback);
            results.push({ vol: v.number, data });
        }
        return results;
    }
    // --- Tracking 数据方法 ---
    async getCharacters(story, vol) {
        const results = await this.collectFromAllVolumes(story, 'character-state.json', { characters: [] }, vol);
    ...
    async getCharacterArc(story, name) {
        const volumes = await this.getVolumes(story);
        const states = [];
        for (const v of volumes) {
            const data = await this.readTrackingJson(story, v.number, 'character-state.json', { characters: [] });
    ...
    async getRelationships(story, vol) {
        const targetVol = vol || await this.findLatestVolume(story);
        if (!targetVol)
            return { nodes: [], edges: [] };
        const charData = await this.readTrackingJson(story, targetVol, 'character-state.json', { characters: [] });
        const relData = await this.readTrackingJson(story, targetVol, 'relationships.json', { relationships: [] });
        const nodes = (charData.characters || []).map((c) => ({
            id: c.name,
            name: c.name,
            faction: c.faction || '',
            role: c.role || '',
        ...
        const edges = (relData.relationships || []).map((r) => ({
            source: r.source,
            target: r.target,
            type: r.type || '',
            status: r.status || '',
    ...
    async getTimeline(story, vol) {
        const results = await this.collectFromAllVolumes(story, 'timeline.json', { events: [] }, vol);
        const events = [];
        for (const { data } of results) {
    ...
    async getPlotThreads(story) {
        const results = await this.collectFromAllVolumes(story, 'plot-tracker.json', { plotlines: [], foreshadowing: [] });
